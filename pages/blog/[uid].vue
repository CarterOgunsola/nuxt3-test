<template>
    ...
    <div v-if="article" class="article-container">
        <div class="my-10">
            <PrimaryButton :to="`/`" text="← Back to Articles" />
        </div>

        <h1 class="h3">{{ article.data.title }}</h1>
        <div class="article-meta">
            <span v-if="article.data.publication_date"> Published on: {{ formatDate(article.data.publication_date) }} </span>
        </div>
    ...
    <div class="flex justify-center items-center h-screen absolute top-0 left-0 w-full" v-else>Loading article...</div>
</template>
